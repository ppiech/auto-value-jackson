...
public abstract class SingleParameterCollectionType extends CollectionType {
  ...
  public void parse(Builder builder, int depth, String setter, Object... setterFormatArgs) {
    ...
    final String instanceCreator = String.format("$T<%s> $L = new $T<%s>()", parameterType.getParameterizedTypeString(),
        parameterType.getParameterizedTypeString());
    final Object[] instanceCreatorArgs =
        expandStringArgs(getTypeName(), parameterType.getParameterizedTypeStringArgs(), collectionVarName,
            getTypeName(), parameterType.getParameterizedTypeStringArgs());
  ...
  public void serialize(MethodSpec.Builder builder, int depth, String fieldName,
    ...
    final Object[] instanceCreatorArgs =
        expandStringArgs(getGenericClass(), parameterType.getParameterizedTypeStringArgs(), collectionVariableName,
            getter);

    final String forLine = String.format("for (%s $L : $L)", parameterType.getParameterizedTypeString());
    final Object[] forLineArgs =
        expandStringArgs(parameterType.getParameterizedTypeStringArgs(), elementVarName, collectionVariableName);
    ...
    parameterType.serialize(builder, depth + 1, collectionVariableName + "Element", processedFieldNames,
