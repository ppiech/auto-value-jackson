...
public class ArrayCollectionType extends CollectionType {
  ...
  public void parse(Builder builder, int depth, String setter, Object... setterFormatArgs) {
    ...
    final String instanceCreator = String.format("$T<%s> $L = new $T<%s>()", arrayType.getParameterizedTypeString(),
        arrayType.getParameterizedTypeString());
    final Object[] instanceCreatorArgs =
        expandStringArgs(List.class, arrayType.getParameterizedTypeStringArgs(), collectionVarName, ArrayList.class,
            arrayType.getParameterizedTypeStringArgs());

    builder.beginControlFlow("if ($L.getCurrentToken() == $T.START_ARRAY)", TypeUtils.JSON_PARSER_VARIABLE_NAME, JsonToken.class)
        .addStatement(instanceCreator, instanceCreatorArgs)
        .beginControlFlow("while ($L.nextToken() != $T.END_ARRAY)", TypeUtils.JSON_PARSER_VARIABLE_NAME, JsonToken.class);
    ...
    if (fieldType.isPrimitive()) {
      ...
      builder.addStatement("$T[] array = $L.toArray(new $T[$L.size()])", fieldType, collectionVarName, fieldType,
          collectionVarName);
    }
  ...
  public void serialize(MethodSpec.Builder builder, int depth, String fieldName,
    ...
    if (arrayType.getTypeName().isPrimitive()) {
      ...
      instanceCreatorArgs =
          expandStringArgs(arrayType.getParameterizedTypeStringArgs(), collectionVariableName, getter);
      forLine = String.format("for (%s $L : $L)", arrayType.getParameterizedTypeString());
      forLineArgs =
          expandStringArgs(arrayType.getParameterizedTypeStringArgs(), elementVarName, collectionVariableName);
    }
    ...
    builder
        .addStatement("$L.writeStartArray()", TypeUtils.JSON_GENERATOR_VARIABLE_NAME)
    ...
    arrayType.serialize(builder, depth + 1, collectionVariableName + "Element", processedFieldNames,
        elementVarName, false, false, "false", writeCollectionElementIfNull);
    ...
    builder
        .endControlFlow()
        .addStatement("$L.writeEndArray()", TypeUtils.JSON_GENERATOR_VARIABLE_NAME)
        .endControlFlow();
  }
